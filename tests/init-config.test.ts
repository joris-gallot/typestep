...
test('init config with ignoredTsErrorCodes', async () => {
  const config = await generateInitialConfig({
    tscOutputFile: tscOutputPath,
    ignoreFiles: false,
...
test('init config with both ignoredFiles and ignoredTsErrorCodes', async () => {
  const config = await generateInitialConfig({
    tscOutputFile: tscOutputPath,
    ignoreFiles: true,
    ignoreTsErrorCodes: true,
...
test('init config with no ignore flags', async () => {
  const config = await generateInitialConfig({
    tscOutputFile: tscOutputPath,
    ignoreFiles: false,
    ignoreTsErrorCodes: false,
