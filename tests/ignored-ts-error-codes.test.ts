...
test('without ignored error codes', async () => {
...
test('with one ignored error code (TS2345)', async () => {
  const { tscErrors } = getTscErrors(parsedTscOutput, {
...
test('with multiple ignored error codes (TS2345, TS7006)', async () => {
  const { tscErrors } = getTscErrors(parsedTscOutput, {
...
test('with non-existent ignored error codes', async () => {
  const { tscErrors } = getTscErrors(parsedTscOutput, {
...
test('with mixed existing and non-existent ignored error codes', async () => {
  const { tscErrors } = getTscErrors(parsedTscOutput, {
...
test('with ignored error codes and ignored files', async () => {
  const { tscErrors } = getTscErrors(parsedTscOutput, {
